...
class UploadHelper
{
    ...
    private static $midFolder = '/images/mid';
    private static $thumbFolder = '/images/thumb';
    ...
    public static function upload($field, $destinationPath = '', $newName = '', $width = 0, $height = 0, $makeOtherSizesImages = true)
    {
        ...
        if ($makeOtherSizesImages === true) {
            ...
            {
                Storage::disk('public')->makeDirectory($midImagePath);
            }
            $med = $imageToResize->resize(self::$midImgWidth, self::$midImgHeight, function ($constraint) {
                $constraint->aspectRatio();
                $constraint->upsize();
            })->save($midImagePath . '/' . $fileName);
            Storage::disk('public')->put($midImagePath.$fileName,$med);
